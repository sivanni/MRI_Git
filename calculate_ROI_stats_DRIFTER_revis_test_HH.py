...
#patient_data = '/opt/MR_data/Silja_Raty/Revis_0007_rs/Results_nodeoblique/Functional_nosearch/data2/'
# potilaiden SVR-analyysin tulokset
#patient_results = '/opt/MR_data/Silja_Raty/Revis_0007_rs/Results_nodeoblique/Connectivity_analysis/data2/'
# esikasitelty verrokkidata
#pilot_data =  '/opt/MR_data/Silja_Raty/'+sub+'/Results_nodeoblique/Functional_nosearch/data3/'
# verrokkien SVR-analyysin tuloksey
#pilot_results = '/opt/MR_data/Silja_Raty/Revis_0007_rs/Results_nodeoblique/Connectivity_analysis/data2/'

pilot_data = '/opt/MR_data/Silja_Raty/Pilot_data/'
pilot_results = '/opt/MR_data/Silja_Raty/Pilot_data/DRIFTER_SVR_noNaN/'

pilots = ['20141101', '20141101_1', '20141101_2', '20141108', '20141108_1', '20141108_2', '20141115', '20141115_2', '20141122', '20141122_2'] #os.listdir(pilot_results)

# Load patient data
subjects = ['Revis_0004_rs']#os.listdir(data_path)

for i in range(0, len(subjects)):

	test_subj = subjects[i]

	#patient_data = '/opt/MR_data/Silja_Raty/'+test_subj+'/Results_nodeoblique/Functional_nosearch/data2/' #turha
	patient_results = '/opt/MR_data/Silja_Raty/'+test_subj+'/Results_nodeoblique/Connectivity_analysis_noNaN_2/data3/'
	...
	anat = '/opt/MR_data/Silja_Raty/'+test_subj+'/Results_nodeoblique/00002_session3/anat/reorient/anat_LR_brain_reoriented.nii.gz'
	...
	for i in range(0,n):
		...
		ROI_corr1 = loadmat(pilot_data + 'DRIFTER_SVR_noNaN/subj' + subj + '_corrcoef_fmri1.mat')['corrcoef_fmri1'][0]
		ROI_corr2 = loadmat(pilot_data + 'DRIFTER_SVR_noNaN/subj' + subj + '_corrcoef_fmri2.mat')['corrcoef_fmri2'][0]
		reprod = loadmat(pilot_data + 'DRIFTER_SVR_noNaN/subj' + subj +'_reprod_coef.mat')['reprod_coef'][0]
	...
	#ids1 = np.argsort(accuracy)
	ids1 = np.argsort(np.mean(accuracy,axis=1))
	accuracy_sorted = accuracy[ids1]
	cort_labels = cort_labels[ids1]
	...
	diff_acc = (test_accuracy-np.mean(accuracy_sorted,axis=1))/np.std(accuracy_sorted,axis=1)
	...
	for i in range(1,roin):
		a= ids1[i]+1
		voxel_ids = np.where(orig_ROI_data==a)
		acc_roi_data[voxel_ids[0],voxel_ids[1], voxel_ids[2]] = diff_acc[i]
	...
	for i in range(0,roin-1):
	    if diff_rep[i] < -3.0:
	    	plt.scatter(test_reproducibility[i], i+1, marker="*")
		labels[i].set_color('red')
	...
	for i in range(1,roin):
		a= ids1[i]+1
		voxel_ids = np.where(orig_ROI_data==a)
		acc_roi_data[voxel_ids[0],voxel_ids[1], voxel_ids[2]] = diff_rep[i]
